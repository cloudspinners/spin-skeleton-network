...
resource "aws_iam_role" "packaging_codebuild_role" {
  name = "${var.component}-${var.estate}_Packager"
...
resource "aws_iam_policy" "packaging_codebuild_policy" {
  name        = "${var.component}-${var.estate}_Packaging_Codebuild_Policy"
  path        = "/service-role/"
  description = "Policies needed by the CodeBuild project for Packaging the ${var.component}-${var.estate} service"
...
resource "aws_iam_policy_attachment" "packaging_codebuild_attachment" {
  name       = "${var.component}-${var.estate}_Packaging_Codebuild_Attachment"
